...
# Untuk konfigurasi lebih spesifik (lebih aman):
# CORS(app, origins=['http://localhost:3000', 'https://domainanda.com'])

# ─────────────────────────────────────────
#  SimpleProtobuf helper
# ─────────────────────────────────────────
...
def get_available_room(hex_data: str):
    """
    FIX: পুরনো কোডে IP/port slice এ off-by-one ছিল।
    এখন field '14' এর value থেকে শেষ ':' দিয়ে split করে IP ও port আলাদা করা হয়।
    """
    try:
...
def parse_server_address(address: str):
    """
    FIX: আগের কোডে hardcoded slice [-6:] / [:-6] ব্যবহার হত,
    যেটা port এর digit সংখ্যা ভেদে ভুল হতো।
    এখন rsplit(':',1) দিয়ে সঠিকভাবে IP ও port আলাদা করা হচ্ছে।
    """
    try:
...
def build_start_packet(account_id: int, timestamp: int, jwt: str, key, iv) -> str:
    """
    FIX: আগের কোডে jwt.encode().hex() করা হত — কিন্তু jwt একটি string,
    সরাসরি jwt.encode('utf-8').hex() করাই সঠিক।
    encrypt_packet এ pass করার আগে সেটাই করতে হবে।
    """
    try:
        # JWT string → bytes → hex → AES encrypt
        jwt_hex = jwt.encode('utf-8').hex()
        encrypted = encrypt_packet(jwt_hex, key, iv)
...
#  Core logic
# ─────────────────────────────────────────
def process_ban(access_token: str):
    import requests

    # ── Step 1: Token inspect ──
    ...
    if not NEW_OPEN_ID or not platform_:
        return False, "Missing open_id or platform in token inspect response"
    ...
    try:
        ...
        try:
            ...
            return False, f"Failed to parse MajorLogin response: {str(e)}"

    if not resp_msg.account_jwt:
        return False, "No JWT received from MajorLogin"
    ...
    if field_21_value:
        ...
        payload_jwt_tmp = extract_jwt_payload_dict(resp_msg.account_jwt)
        if not payload_jwt_tmp:
            return False, "Failed to decode JWT for timestamp"
        exp = int(payload_jwt_tmp.get("exp", 0))
        ts = Timestamp()
        ts.FromNanoseconds(exp * 1_000_000_000)
        timetamp = ts.seconds * 1_000_000_000 + ts.nanos

    # ── Step 4: GetLoginData ──
    GetLoginData_url = "https://clientbp.ggblueshark.com/GetLoginData"
    ...
    try:
        r2 = requests.post(GetLoginData_url, headers=GetLoginData_headers, data=enc_data, timeout=12, verify=False)
    ...
    try:
        ...
        if json_result:
            parsed_login = json.loads(json_result)
            if '14' in parsed_login and 'data' in parsed_login['14']:
                online_address = parsed_login['14']['data']
                # FIX: rsplit(':',1) — port digit count এ আর সমস্যা নেই
                online_ip, online_port = parse_server_address(online_address)
                if not online_ip or not online_port:
                    return False, f"Could not parse server address: {online_address}"
            else:
                return False, "Field '14' not found in GetLoginData response"
        else:
            return False, "Failed to parse GetLoginData response"
    except Exception as e:
        return False, f"Error processing GetLoginData response: {str(e)}"

    # ── Step 6: Build & send packet ──
    ...
    if account_id == 0:
        return False, "account_id not found in JWT"
    ...
    if not final_token_hex:
        return False, "Failed to build start packet"

    try:
        payload_bytes = bytes.fromhex(final_token_hex)
        server_response = send_once(online_ip, online_port, payload_bytes, recv_timeout=5.0)
        if server_response:
            return True, "Account BAN successfully executed"
        else:
            return False, "No response from game server"
    except Exception as e:
...
def index():
    return jsonify({
        "status": "running",
        "service": "Free Fire Ban Tool",
        "version": "1.0.0",
        "endpoints": {
            "GET /ban?token=ACCESS_TOKEN": "Send ban using access token",
            "POST /ban": "Send ban with JSON body: {'token': 'ACCESS_TOKEN'}",
            "GET /": "API information"
        },
        "creator": "CGU TEAM"
    })
...
@app.route('/ban', methods=['GET', 'POST', 'OPTIONS'])
def ban_endpoint():
    ...
    if request.method == 'GET':
        ...
        access_token = body.get('token', '').strip()
    ...
    try:
        success, message = process_ban(access_token)
        return jsonify({
            "success": success,
            "message": message,
